...
```bash
./scripts/cli.js chat "<问题>"
```

### 图文对话（带图片）

```bash
./scripts/cli.js chat "<问题>" --image <图片URL或base64>
```

### 文生图

```bash
./scripts/cli.js image "<描述>" --ratio <比例> --style <风格>
```

### 图生图

```bash
./scripts/cli.js image "<描述>" --image <原图URL或base64> --ratio <比例> --style <风格>
```

### Token 检测

```bash
./scripts/cli.js token-check <token>
```
